...
base:
	DOCKER_BUILDKIT=1 docker buildx build ${ATTEST} ${PROVENANCE} -t gerph/rpcemu-base:${BASE_DOCKER_TAG} --build-arg BASE_DOCKER_TAG=${BASE_DOCKER_TAG} --build-arg RPCEMU_VERSION=${RPCEMU_VERSION} -f base.Dockerfile .

ro37: base ${VNCRESIZE}
	DOCKER_BUILDKIT=1 docker buildx build ${ATTEST} ${PROVENANCE} -t gerph/rpcemu-3.7:${BASE_DOCKER_TAG} --build-arg BASE_DOCKER_TAG=${BASE_DOCKER_TAG} --build-arg RPCEMU_VERSION=${RPCEMU_VERSION} -f ro37.Dockerfile .

ro5: base ${VNCRESIZE}
	DOCKER_BUILDKIT=1 docker buildx build ${ATTEST} ${PROVENANCE} -t gerph/rpcemu-5:${BASE_DOCKER_TAG} --build-arg BASE_DOCKER_TAG=${BASE_DOCKER_TAG} --build-arg RPCEMU_VERSION=${RPCEMU_VERSION} -f ro5.Dockerfile .
